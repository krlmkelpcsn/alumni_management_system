    ...
    <header class="">
            <div class="head container-fluid h-100">
                <div class="row h-100 align-items-center justify-content-center text-center">
                    ...
                    <h1 class="mt-3 display-4 fw-bold">Announcements</h1>
        ...
        <div class="col-md-6 col-lg-4 mb-4">
            <div class="card h-100 shadow-sm">
                <div class="card-body">
                    ...
                    <button class="btn btn-primary btn-sm float-right view_announcement" data-id="<?php echo $row['id']; ?>">View </button>
                </div>
            </div>
        </div>
        <?php endif; ?>
...
<script>
    ...
    $('#new_forum').click(function() {
        uni_modal("New Forum Entry", "manage_announcement.php", 'mid-large');
    });
    // $('#new_forum').click(function(){
    //     uni_modal("New Topic","manage_forum.php",'mid-large')
    // })
    $('.view_announcement').click(function(){
       location.replace('index.php?page=view_announcement&id='+$(this).attr('data-id'))
    })
    $('.edit_forum').click(function(){
        uni_modal("Edit Topic","manage_announcement.php?id="+$(this).attr('data-id'),'mid-large')
    })
    $('.gallery-img img').click(function(){
        viewer_modal($(this).attr('src'))
    })
     $('.delete_forum').click(function(){
        _conf("Are you sure to delete this Announcement?","delete_announcement",[$(this).attr('data-id')],'mid-large')
    })

    function delete_forum($id){
        start_load()
        $.ajax({
            url:'admin/ajax.php?action=delete_announcement',
            method:'POST',
            data:{id:$id},
